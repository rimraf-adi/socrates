# Generator-Critic Agent System

Two-agent iterative refinement system. A **generator** produces responses (with web search via SearXNG), a **critic** reviews and provides feedback, and the loop repeats for N iterations.
...
- **Generator**: receives task + critic feedback, searches the web for context, produces a response
- **Critic**: evaluates the response on accuracy, completeness, clarity, depth, and actionability
...
- **SearXNG** running at `http://localhost:8080` (via `docker-compose up` from project root)
